        ...
        public static void Action(string Instructions)
        {
            ...
            if (usedKeywords[0].Contains("go"))
            {
                foreach (var exit in CurrentLocation.exits)
                {
                    ...
                    {
                        Console.WriteLine("The door to the {0} is locked, do you have a key? (check inventory)", exit.Direction);
                        break;
                    }
                    else if(!(usedKeywords[1] == exit.Direction))
                    {
                        Console.WriteLine("There is no exit in that direction");
                    }
                }
            }
            else if (usedKeywords[0].Contains("use"))
            {
            ...
            else if (usedKeywords[0].Contains("take"))
            {
            ...
            else if (usedKeywords[0].Contains("drop"))
            {
                for (int i = 0; i < Inventory.Count; i++)
                {
            ...
            else if (usedKeywords[0].Contains("inspect"))
            {
                for (int i = 0; i < CurrentLocation.items.Count; i++)
                {
                    if (usedKeywords[1] == CurrentLocation.items[i].Name)
            ...
            else if (usedKeywords[0].Contains("look"))
            {
